...
#' according to the Metapsy [data standard](https://docs.metapsy.org/data-preparation/format/). This argument is `NULL` by default,
#' which means that only domain-specific and overall RoB ratings are generated from the RoB extraction
#' sheet provided to `rob.data`.
#'
...
#' study-level (`rob_study_lvl`) overall RoB judgments. `NULL` if no database was provided.
#' - `miss.studies`. A vector of studies that appear in the database, but were not
#' found in the extraction sheet. `NULL` if no database was provided.
#' - `miss.studies.rob`. A vector of studies that appear in the extraction sheet, but
...
createRobRatings = function(rob.data, database = NULL) {
  ...
  data = database; has.data = !is.null(data)[1]
  ...
  if (has.data) {
    ...
    ifelse(ifelse(p.attr.arm1<.05, "Yes/PY", "No/PN") == "Yes/PY" &
             ifelse(p.attr.arm2<.05, "Yes/PY", "No/PN") == "Yes/PY",
           "Yes/PY", "No/PN") %>% {.[is.na(d.merge$attr_arm1)]="NI";.} %>% 
                            {.[is.na(d.merge$attr_arm2)]="NI";.} %>%
                            {.[is.na(d.merge$rand_arm1)]="NI";.} %>% 
                            {.[is.na(d.merge$attr_arm2)]="NI";.}-> d.merge$d3_10
    ifelse(ifelse(p.attr.arm1<=.3, "Yes/PY", "No/PN") == "Yes/PY" &
             ifelse(p.attr.arm2<=.3, "Yes/PY", "No/PN") == "Yes/PY",
           "Yes/PY", "No/PN") %>% {.[is.na(d.merge$attr_arm1)]="NI";.} %>% 
                            {.[is.na(d.merge$attr_arm2)]="NI";.} %>% 
                            {.[is.na(d.merge$rand_arm1)]="NI";.} %>% 
                            {.[is.na(d.merge$attr_arm2)]="NI";.}-> d.merge$d3_13
    ifelse(d.merge$rating=="self-report", "Yes/PY", "No/PN") %>% 
      {.[is.na(d.merge$rating)]="NI";.} -> d.merge$d4_16
    ifelse(d.merge$rating=="self-report", "No/PN", "Yes/PY") %>% 
    ...
    # Loop through RoB rating algorithms if has.data=FALSE
    apply(rob.data, 1, function(x){
      data.frame(
        d1 = domain1Algorithm(x[["d1_1"]], x[["d1_2"]], x[["d1_3"]], x[["d1_4"]]),
        d2 = domain2Algorithm(x[["d2_5"]], x[["d2_6"]], x[["d2_7"]], x[["d2_8"]], x[["d2_9"]]),
        d3 = domain3Algorithm(x[["d3_10"]], x[["d3_11"]], x[["d3_12"]], x[["d3_13"]], x[["d3_14"]]),
        ...
        d5 = domain5Algorithm(x[["d5_19"]], x[["d5_20"]], x[["d5_21"]], x[["d5_22"]], x[["d5_23"]], x[["d5_24"]])) %>% 
        {.$.rob = overallAlgorithm(.$d1, .$d2, .$d3, .$d4, .$d5);.}
    }) %>% do.call(rbind, .) %>% cbind(rob.data[!colnames(rob.data) %in% colnames(.)], .) %>% 
      within({rob = .rob; .rob = NULL}) -> rob.data
    miss.studies = NULL; miss.studies.rob = NULL
    
  }
  
  ret.obj = list(rob.data = rob.data,
                 database = database,
