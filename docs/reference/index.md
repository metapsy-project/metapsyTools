...
- [`rr.binary()`](rr.binary.md) : Calculate the log-risk ratio using
  binary outcome data
- [`rom.m.sd()`](rom.m.sd.md) : Calculate the log ratio of means (ROM)
  using means and standard deviations
- [`rom.change.m.sd()`](rom.change.m.sd.md) : Calculate the log ratio of
  means (ROM) using within-group change data
...
- [`sdReference`](sdReference.md) : The 'sdReference' dataset
...
- [`addTrialArmInfo()`](addTrialArmInfo.md) : Add information that
  ...
  dataset
- [`listSDReferences()`](listSDReferences.md) : Display available
  SD-reference instruments
